...
//Events
fontsElements.forEach((el) => {
  el.addEventListener('click', toggleFSButtons);
});
textColorElements.forEach((el) => {
  el.addEventListener('click', toggleTCButtons);
});
backgroundColorElements.forEach((el) => {
  el.addEventListener('click', toggleBGButtons);
});
...
function toggleTCButtons(e) {
  e.preventDefault();
  const currentElement = e.target;
  const textColor = currentElement.dataset.textColor;
...
function changeSateAtFSButton(size, el) {
  fontsElements.forEach((el) => el.classList.remove('font-size_active'));
  el.classList.add('font-size_active');
}
function changeStateAtButton(color, el) {
  const elements = el.closest('.book__control').querySelectorAll('.color');
...
function changeColorParagraph(color, type = 'color') {
  if (color === 'black') {
    bookElement.classList.remove(`book_${type}-${type === 'bg' ? 'white' : 'whitesmoke'}`, `book_${type}-gray`);
    bookElement.classList.add(`book`, `book_${type}-${color}`);

  } else if (color === 'gray') {
    bookElement.classList.remove(`book_${type}-${type === 'bg' ? 'white' : 'whitesmoke'}`, `book_${type}-black`);
    bookElement.classList.add(`book`, `book_${type}-${color}`);
  } else {
